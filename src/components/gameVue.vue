<template>
  <div class="lg:w-2/5 md:w-full aspect-square mx-auto text-white relative">
    ...
    <div
      :class="{ ' border-boundary grid grid-cols-10': !gameFinished }"
...
function placeApple() {
  // get empty cells
  let emptyCells = board.value
    .map((cell) => (cell !== "snake" ? cell : undefined))
    .filter((cell) => cell !== undefined);
  // put the apple to random place inside of empty cells
  appleIndex = emptyCells[Math.floor(Math.random() * emptyCells.length)];
...
//touch listeners for mobile-tablet devices
